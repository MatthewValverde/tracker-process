...
extern  "C" {
	...
	int EXPORT_API ULS_TrackerInit(char *pSrcModelPath, char *pSrckActivationKey, int nbrtracker=5, int det_type=1, int min_face=40, int frame_rate=30);//return value 1:Success (<0):false
	...
	//3: bgra2gray
	//4: rgba2gray
	int EXPORT_API ULS_TrackerProcessByte(void *inputdata, int imgw, int imgh, int mode, double imgAngle, float *pYawAngle);//return value (<0):error (-50):No face (>0):Success
	int EXPORT_API ULS_TrackerProcessRect(void *pSrcdata, int &srcw, int &srch,
							int &fx, int &fy, int &fw, int &fh, float *points, float *pose);

	int EXPORT_API ULS_AddFaces(const int n, const int top[], const int left[], const int width[], const int height[], const float yaw[]);
	int EXPORT_API ULS_GetFaceDetRect(int faceNbr, int &fx, int &fy, int &fw, int &fh);
	...
	// ------------------------------
#endif
	bool EXPORT_API ULS_CheckKeyEffect(char *pSrckActivationKey, char *pSrcpath);
	
	void EXPORT_API ULS_SetTwoThresholds(const float thresh, const float thresh_det);
